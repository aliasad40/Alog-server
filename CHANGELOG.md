...
### Added
- **Recent logs load automatically.** Opening Log Search now shows the latest
  entries with no filters set. This uses a dedicated `/api/search/latest`
  endpoint rather than an empty search, because the sorting key is
  `(public_ip, public_port, timestamp)` and an unfiltered `ORDER BY timestamp
  DESC` over 24 hours would scan the whole range every time somebody opened
  the page. The window starts at 15 minutes (configurable) and only widens —
  to 1 hour, 1 day, then 1 week — if it comes back empty, so a quiet or
  freshly-installed system still shows something while a busy one never pays
  for more than it needs.

- **Services tab.** Start, stop and restart the receiver, worker, API, Redis,
  ClickHouse and nginx from the browser. Each card states what restarting it
  actually costs before you commit — restarting the receiver loses logs in
  flight, restarting the worker loses nothing.

  This is a privilege boundary, so it is scoped tightly: unit names come from
  a fixed allow-list and never from the request, commands are executed as an
  argument vector with no shell, and sudo is granted per exact command with no
  wildcards. The sudoers file is generated by the same module that issues the
  commands, validated with `visudo -c` before installation, and a test asserts
  the two cannot drift apart.

- **Diagnostics tab.** Read the receiver, worker, API and maintenance logs in
  the browser, filtered by severity, with errors and warnings colour-coded and
  optional auto-refresh. Filenames come from a fixed map, so no amount of
  `../` in the query string reaches another file.

- **Configurable display timezone.** Timestamps no longer render with a
  `+00:00` suffix. Set an IANA zone (`Asia/Karachi`, `Europe/London`) in
  Settings and it applies to displayed timestamps *and* to how your search
  input is interpreted — typing 18:00 in Karachi now searches 13:00 UTC
  instead of 18:00 UTC.

  Only IANA names are accepted; fixed offsets are rejected because they get
  daylight saving wrong twice a year. Tests cover DST transitions, negative
  offsets and half-hour zones.

### Changed
- `timestamp` and `session_end_time` are pinned to `DateTime('UTC')`, and the
  worker inserts UTC-aware values. Previously a naive datetime was interpreted
  in the ClickHouse server's own timezone — a setting this application does
  not control. Migration `001-pin-timestamps-to-utc.sql` is metadata-only and
  completes instantly on any table size; stored instants are unchanged.

### Fixed
- **`render_schema` split statements on semicolons inside SQL comments.**
  Comments were stripped *after* splitting on `;`, so a comment containing a
  semicolon cut a statement in half and leaked its tail into the next one,
  producing a syntax error from `init-db` nowhere near its cause. Comments are
  now stripped first. Found by a schema test after a new comment triggered it.
