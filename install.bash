...
configure_service_control() {
  step "Authorising service control"
  # The web interface can restart services. That is a privilege boundary, so
  # sudo is scoped to exact argument vectors -- one line per (action, unit),
  # no wildcards, nothing else. The list is generated by the same module that
  # issues the commands, so the two cannot drift apart.
  local sudoers=/etc/sudoers.d/network-log-server
  "${VENV_DIR}/bin/python" - <<PYEOF > "${sudoers}.tmp"
import sys
sys.path.insert(0, "${INSTALL_DIR}/backend")
from app.services.control import sudoers_lines
print("\n".join(sudoers_lines("${APP_USER}")))
PYEOF
  run chmod 0440 "${sudoers}.tmp"
  run chown root:root "${sudoers}.tmp"
  # visudo -c on the candidate file: a malformed sudoers file locks out sudo
  # for the whole machine, so it is validated before being put in place.
  if visudo -cf "${sudoers}.tmp" >>"$INSTALL_LOG" 2>&1; then
    run mv "${sudoers}.tmp" "$sudoers"
    ok
  else
    rm -f "${sudoers}.tmp"
    printf '%sSKIPPED%s\n' "$DIM" "$RESET"
    warn "sudoers validation failed; the Services page will not be able to restart units.
       Everything else works. See $INSTALL_LOG."
  fi
}
